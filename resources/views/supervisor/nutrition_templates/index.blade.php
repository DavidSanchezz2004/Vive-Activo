...
<div class="d-card" style="margin-bottom: 16px;">
    <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
        <div>
            <h2 style="margin:0;">Plantillas de planes nutricionales</h2>
            <p style="margin:6px 0 0;color:var(--d-muted);">Crea y administra plantillas reutilizables.</p>
        </div>
        <a class="d-btn d-btn-primary" href="{{ route('supervisor.nutrition_templates.create') }}">Nueva plantilla</a>
    </div>
</div>
            ...
            <tbody>
                @forelse($templates as $tpl)
                    <tr>
                        ...
                        <td style="color:var(--d-muted);">{{ $tpl->phase ?? '—' }}</td>
                        ...
                        <td>
                            <div style="display:flex;gap:8px;flex-wrap:wrap;">
                                <a class="d-btn d-btn-outline" href="{{ route('supervisor.nutrition_templates.edit', $tpl) }}">Editar</a>
                                <form method="POST" action="{{ route('supervisor.nutrition_templates.destroy', $tpl) }}" onsubmit="return confirm('¿Eliminar esta plantilla?');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="6" style="color:var(--d-muted);">No hay plantillas registradas.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
