...
@section('title', 'Nuevo Plan Nutricional - Supervisor')
...
@section('content')
  ...
  <form action="{{ route('supervisor.pacientes.nutrition_plans.store', $patient) }}" method="POST" enctype="multipart/form-data">
    @csrf
        ...
        @foreach($mealTimes as $key => $label)
          <div class="meal-section" id="meal-{{ $key }}">
            ...
            <div class="meal-body" id="body-{{ $key }}">
              ...
              <button type="button" onclick="addItem('{{ $key }}')" class="d-btn d-btn-outline" style="font-size:13px;margin-top:4px;">
...
<script>
  ...
  function addItem(mealKey, prefill = null) {
  ...
  function applyTemplate(templateKey) {
    ...
    Object.entries(tpl.items || {}).forEach(([mealKey, items]) => {
      (items || []).forEach((item) => addItem(mealKey, item));
    });
  }
